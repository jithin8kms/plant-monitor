...
char ssid[] = "MagentaWLAN-YL7R";          // your network SSID (name)
...
sensor moisture_sensor_1(A0);   // Analog pin connected to the sensor's output
...
{  
  ...
  if (WiFi.status() != WL_CONNECTED) // retry wifi connection
  {
    init_wifi_connection();
  }
  else
...
{
  moisture_sensor->value = analogRead(moisture_sensor->pin);  // Read the analog value from the sensor
  moisture_sensor->percent = map(moisture_sensor->value, 800, 400, 0, 100);
  moisture_sensor->percent = constrain(moisture_sensor->percent, 0, 100);  // Constrain the value to 0-100%  
}

void print_sensor_data(sensor* moisture_sensor)
{
    //Serial.print("Soil Moisture Level: ");
  Serial.print(moisture_sensor->value);          // Print the sensor value to the Serial Monitor
  Serial.print("\t");
  Serial.println(moisture_sensor->percent);          // Print the sensor value to the Serial Monitor
  Serial.print("\t");
}